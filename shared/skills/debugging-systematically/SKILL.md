...
1. **Create Failing Test Case**
   ...
   - Use the `writing-tests-first` skill for writing proper failing tests
...
3. **Verify Fix**
   ...
   - Use the `verifying-before-claiming` skill before claiming success
...
## Phase 5: What the Investigation Leaves Behind

Finding the root cause and fixing it, then leaving only a commit behind,
throws away the expensive half. The time the investigation cost bought
understanding, not just the changed lines — and understanding evaporates with
...
`systematic-debugging`, with a fifth phase for routing what the
investigation learned onto the board.
