...
Subagent (general-purpose):
  description: "Re-review bite <BITE_ID> fix round R"
  model: [MODEL — REQUIRED: choose per SKILL.md Model Selection; an omitted
         model silently inherits the session's most expensive one]
  prompt: |
    You are re-reviewing one bite's fix round. A previous review produced
    ...
    Call get_slice("<REF>") and list_bites(<SLICE_ID>). The bite under
    verification is id <BITE_ID>; its body is what was requested.
    ...
    **Diff file:** [DIFF_FILE]
    ...
    not block this bite and does not extend the loop. A broad whole-branch
    review happens after every bite is complete.
...
- `<REF>` / `<SLICE_ID>` / `<BITE_ID>` — REQUIRED: the same board address the
  implementer and the first reviewer worked from
