...
Subagent (general-purpose):
  description: "Implement bite <BITE_ID>: [bite title]"
  model: [MODEL — REQUIRED: choose per SKILL.md Model Selection; an omitted
         model silently inherits the session's most expensive one]
  prompt: |
    You are implementing one bite of tuckit slice <REF>.

    ## Your Requirements

    Call get_slice("<REF>") and list_bites(<SLICE_ID>). Your bite is id
    <BITE_ID>. Its body is your requirements — use the exact values in it
    verbatim. The slice's Constraints section binds you too; read it before
    you start. Implement that bite and nothing else — the other bites belong
    to other subagents.

    Do not write to the board. Your bite's status is the controller's to set.

    ## Context

    [Scene-setting: where this fits, dependencies, architectural context, and
    the interfaces earlier bites produced that the board cannot know]
    ...
    - Anything unclear in the bite body
    ...
    **While you work:** If you encounter something unexpected or unclear, **ask
    ...
    - Follow the file structure the bite defines
    - Each file should have one clear responsibility with a well-defined
      interface
    - If a file you're creating is growing beyond the bite's intent, stop and
    ...
    **STOP and escalate when:**
    - The bite requires architectural decisions with multiple valid approaches
    - You need to understand code beyond what was provided and can't find
      clarity
    - You feel uncertain about whether your approach is correct
    - The bite involves restructuring existing code in ways it didn't anticipate
    ...
    **How to escalate:** Report back with status BLOCKED or NEEDS_CONTEXT.
    ...
    a more capable model, or split the bite into smaller pieces.
    ...
    **Completeness:**
    - Did I fully implement everything the bite asked for?
    ...
    **Testing:**
    ...
    If the bite review finds issues, you will be resumed with the findings. Fix
    them, re-run the tests that cover the amended code, and append a fix report
    ...
    - **TDD Evidence** (if TDD was required for this bite):
    ...
    correctness. Use BLOCKED if you cannot complete the bite. Use NEEDS_CONTEXT
