...
description: "Use before writing implementation code for any feature or bug fix — the practice of test-driven development: write the failing test first, watch it fail, then write the minimal code to pass"
---
...
**Announce at start:** "I'm using writing-tests-first for this change."
...
- **Artifacts that do not execute** — documentation, prompts, pure styling.
  ...
  code; verify a document written for an agent through that agent's behavior.
...
Confirm: the test fails (it does not error), the message is the one you expected,
and it fails because the behavior is missing — not because of a typo.
...
Bug found? Find the root cause first — `debugging-systematically` — before
writing the failing test that reproduces it. Then follow the TDD cycle: the
test proves the fix and prevents regression.
...
`test-driven-development`, with a fourth exception for artifacts that don't
execute and a constraints-writing habit for agreed exceptions.
